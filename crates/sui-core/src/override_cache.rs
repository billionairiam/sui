...
impl InputLoaderCache<'_> {
    ...
    fn read_receiving_objects_for_signing(
        ...
        for objref in receiving_objects {
            ...
            let key = FullObjectKey::new(FullObjectID::new(*object_id, Some(version.clone())), version.clone());

            if self.have_received_object_at_version(key, epoch_id) {
...
impl ObjectCacheRead for InputLoaderCache<'_> {
    ...
    fn get_marker_value(
        &self,
        object_id: FullObjectKey,
        epoch_id: EpochId,
    ) -> Option<sui_types::storage::MarkerValue> {
        self.loader
            .cache
            .get_marker_value(object_id, epoch_id)
    }
