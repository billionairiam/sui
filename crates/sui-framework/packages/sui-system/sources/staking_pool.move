...
public(package) fun redeem_fungible_staked_sui(
    ...
    let fungible_staked_sui_data = pool.fungible_staked_sui_data_mut();
...
macro fun mul_div($a: u64, $mul: u64, $div: u64): u64 {
    (($a as u128) * ($mul as u128) / ($div as u128)) as u64
}
...
public(package) fun convert_to_fungible_staked_sui(
    ...
    if (!pool.extra_fields.contains(FungibleStakedSuiDataKey {})) {
        pool
            .extra_fields
            .add(
        ...
        let fungible_stake_data = pool.fungible_staked_sui_data_mut();
...
fun fungible_staked_sui_data_mut(pool: &mut StakingPool): &mut FungibleStakedSuiData {
...
public entry fun join_staked_sui(self: &mut StakedSui, other: StakedSui) {
    assert!(self.is_equal_staking_metadata(&other), EIncompatibleStakedSui);
...
public use fun pool_token_exchange_rate_at_epoch as StakingPool.exchange_rate_at_epoch;
...
fun test_calculate_fungible_staked_sui_withdraw_amount(
    ...
    let max_principal = exchange_rate.get_sui_amount(fungible_staked_sui_data_total_supply as u64);
    ...
    let (
        principal_amount,
        rewards_amount,
    ) = exchange_rate.calculate_fungible_staked_sui_withdraw_amount(
        fungible_staked_sui_value,
        fungible_staked_sui_data_principal_amount,
        fungible_staked_sui_data_total_supply,
    );

    let expected_out = exchange_rate.get_sui_amount(fungible_staked_sui_value as u64);
