    ...
    if is_tidehunter {
        ...
        TokenStream::from(quote! {
            ...
            impl <
                ...
                > #intermediate_db_map_struct_name #generics {
                /// Opens the tables in read-only mode but returns an instance of the intermediate struct.
                /// This is used internally to create a read-only handle.
                #[allow(unused_parens)]
                pub fn open_tables_read_only_as_rw_impl(
                    path: std::path::PathBuf,
                    metric_conf: typed_store::rocks::MetricConf,
                ) -> Self {
                    // This creates a temporary path for the secondary instance, which is required for read-only mode.
                    let p: std::path::PathBuf = tempfile::tempdir()
                        .expect("Failed to open temporary directory")
                        .into_path();

                    // Now we call the primary open function, but tell it to open as a secondary (read-only)
                    // by providing a path in the `as_secondary_with_path` argument.
                    Self::open_tables_impl(
                        path,
                        Some(p), // This is the key to opening in read-only mode.
                        metric_conf,
                        None,
                        None,
                        false,
